...
def get_page_view(relevant_docs, wid2pv):
    return [(doc, wid2pv[doc]) for doc in relevant_docs if doc in wid2pv]
...
def get_normelize_union(all_relevent_docs, *args):
    ...
    for feature in args:
        ...
        for doc in feature:
            doc_id_score[doc][i] = feature[doc] / max_val
    return doc_id_score
...
def GetResult(q, idx_title, idx_title2, idx_body, idx_body2, bm25_title, bm25_title2, bm25_body, bm25_body2, pr, pv):
    query = get_tokens(q)
    ...
    num_match_title2 = get_top_n(
        [(relevant_doc, get_num_of_match_binary(query, title_dict2, relevant_doc)) for relevant_doc in title_docs2])
    # num_match_body2 = get_top_n([(relevant_doc,get_num_of_match(query, body_dict2, relevant_doc)) for relevant_doc in body_docs2])

    all_relevent_docs = set().union(*[body_docs, title_docs, title_docs2])

    pr_score = get_top_n(get_page_rank(all_relevent_docs, pr))
    pv_score = get_top_n(get_page_view(all_relevent_docs, pv))

    norm_scores = get_normelize_union(all_relevent_docs,num_match_title, num_match_body, bm25_body_score,
                                      cos_simi_body_score, num_match_title2, pr_score, pv_score)
...
#     res_simple = sorted((result.keys()), key=lambda x: builtins.sum(result[x]), reverse = True)
#     doc = [i for i in train_queries[i]]
#     avg_model.append(average_precision(doc, res_model))
#     avg_smp.append(average_precision(doc, res_simple))
#
#
# print(builtins.sum(avg_model)/30)
# print(builtins.sum(avg_smp)/30)
#
#
# # In[ ]:
#
...
# train_queries.keys()
# for i in range(30):
#     print(avg_model[i], list(train_queries.keys())[i])
#
#
# # In[36]:
#
...
#
#
# bm25_title_score,bm25_body_score,cos_simi_title_score,cos_simi_body_score,pr_score,pv_score,num_match_title,num_match_body)
#
#
# # In[ ]:
#
